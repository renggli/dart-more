...
void verify(
  ...
  for (final iterator = excluded.runes.iterator; iterator.moveNext();) {
    expect(
      matcher(iterator.current),
      isFalse,
      reason: '${unicodeCodePointPrinter(iterator.current)} should not match',
    );
  }
...
void main() {
    ...
    group('script', () {
      ...
      test('distinct', () {
        ...
        for (var charCode = 0; charCode <= 0x10ffff; charCode++) {
          ...
          expect(matches, 1, reason: '$charCode is in $matches script classes');
        }
      });
    });
  });
  ...
  group('pattern', () {
    ...
    test('astral plane', () {
      ...
      expect(
        '\u{10000}'.replaceAllMapped(pattern, (match) => '!${match[0]}!'),
        '\u{10000}',
      );
