...
import 'lexicographical.dart';
import 'natural.dart';
import 'nulls_first.dart';
import 'nulls_last.dart';
import 'reversed.dart';
...
abstract class Ordering<T> with ToStringPrinter {
  ...
  Predicate1<T> equalTo(T a) => (T b) => compare(a, b) == 0;

  /// Creates a predicate that evaluates to `true` for values equal to [a].
  Predicate1<T> notEqualTo(T a) => (T b) => compare(a, b) != 0;

  /// Creates a predicate that evaluates to `true` for values smaller than [a].
  Predicate1<T> lessThan(T a) => (T b) => compare(a, b) > 0;

  /// Creates a predicate that evaluates to `true` for values at most [a].
  Predicate1<T> lessThanOrEqualTo(T a) => (T b) => compare(a, b) >= 0;
